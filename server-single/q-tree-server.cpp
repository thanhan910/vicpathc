...
public:
    Server(const std::string& address, unsigned short port)
        : endpoint_(net::ip::make_address(address), port), acceptor_(ioc_, endpoint_), quadtree_()
    {
        // Generate the quadtree
        quadtree_.gen_quadtree();
        quadtree_.test_quadtree();
    }
...
private:
    ...
    void handle_request(const http::request<http::string_body>& req, http::response<http::string_body>& res)
    {
        ...
        else if (req.method() == http::verb::get && req.target().starts_with("/quadtree/segments"))
        {
            ...
            auto params = parse_query_xy(query);
            if (!params)
            {
                res.result(http::status::bad_request);
            ...
            Point p = {x, y};
            auto [nearestSegment, closestPoint, minDistance, quads] = quadtree_.find_nearest_segment(p);
            res.result(http::status::ok);
...
// http://localhost:8080/quadtree/segments?x=144.866&y=-37.751275
